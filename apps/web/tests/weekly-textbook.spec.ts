...
test('@weekly @textbook display: learner switcher works', async ({ page }) => {
  ...
  // Click on Learner 1 Note to view it
  await page.getByRole('button', { name: 'Learner 1 Note' }).click();
  ...
  // Click on Learner 2 Note to view it
  await page.getByRole('button', { name: 'Learner 2 Note' }).click();
  
  // Verify learner-2's note is displayed
  await expect(page.getByRole('heading', { name: 'Learner 2 Note' })).toBeVisible();
  
  // Verify learner-1's note heading is not displayed (different unit selected)
